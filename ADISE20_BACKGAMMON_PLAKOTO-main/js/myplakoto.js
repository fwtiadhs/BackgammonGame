...
function draw_empty_board(p) {
	...
	if(p!='B') {p='W';}
	var draw_init = {
		'B': {i1:33,i2:0,istep:-1,j1:1,j2:13,jstep:1},
		'W': {i1:1,i2:34,istep: 1, j1:1,j2:13,jstep:1}
	};
	var s=draw_init[p];
	var t='<table class="table2" id="plakoto_table" >';
	for(var i=s.i1;i!=s.i2;i+=s.istep) {
		...
		t += '<tr>';
			...
			for(var j=s.j1;j!=s.j2;j+=s.jstep) {
				t += '<td class="plakoto_keli" id="keli_'+j+'_'+i+'">'+'</td>'; 
			}
		}
	...
	$('#plakoto_board').html(t);
	$('.plakoto_keli').click(click_on_piece);
}
function fill_board() {
...
function reset_board() {
	$.ajax({url: "plakoto.php/board/", headers: {"X-Token": me.token}, method: 'POST',  success: window.location = 'index.html' });
	//$('.move_div').hide();
...
/*function fill_board_by_data(data) {
	...
	for(var i=0;i<33;i++) {
		if(i!=15 && i!==17) {
		var o = data[i];
		var id = '#keli_'+ o.x +'_' + o.y;
		var c = (o.piece!=null)?o.piece_color + o.piece:'';
		var pc= (o.piece!=null)?'piece'+o.piece_color:'';
		var im = (o.piece!=null)?'<center>'+'<img class="piece '+pc+'" src="img/'+c+'.png">'+'</center>':'';
		$(id).addClass(o.b_color+'_keli').html(im);
	}
...
function fill_board_by_data(data) {
	...
	for(var i=0;i<data.length;i++) {
		...
		var c = (o.piece!=null)?o.piece_color + o.piece:'';
		var pc= (o.piece!=null)?'piece'+o.piece_color:'';
		var im = (o.piece!=null)?'<img class="piece '+pc+'" src="img/'+c+'.png">':'';
		$(id).addClass(o.b_color+'_keli').html(im);
...
function update_info(){
	$('#game_info').hide();
	if(me.username!=null){
	$('#game_info').show().html("Piece Color: "+me.piece_color+"<br> Username: "+me.username +'<br>Game state: '+game_status.status+'<br> Now Playing: '+ game_status.p_turn);
}
}
function do_move() {
	var s = $('#the_move').val();
...
function update_moves_selector() {
	$('.plakoto_keli').removeClass('pmove').removeClass('tomove');
	...
	for(i=0;i<board.length;i++) {
		if(board[i].x==a[0] && board[i].y==a[1] && board[i].moves && Array.isArray(board[i].moves)) {
			for(m=0;m<board[i].moves.length;m++) {
				...
				var id = '#keli_'+ board[i].moves[m].x +'_' + board[i].moves[m].y;
				$(id).addClass('pmove');
			}
		}
	}
...
function start_dragging ( event, ui ) {
	...
	for(var i=0;i<board.length;i++) {
		if(board[i].x==a[1] && board[i].y==a[2] && board[i].moves && Array.isArray(board[i].moves)) {
			for(var m=0;m<board[i].moves.length;m++) {
				$('#the_move_dest').append('<option value="'+board[i].moves[m].x+' '+board[i].moves[m].y+'">'+board[i].moves[m].x+' '+board[i].moves[m].y+'</option>');
				var id = '#keli_'+ board[i].moves[m].x +'_' + board[i].moves[m].y;
...
function rollDice(){
	var zariaText = document.getElementById("idZari");
	$.get("plakoto.php/zaria/")
			.done((data) => {
				zariaText.innerHTML = data;
			})
